...
with st.sidebar:
    ...
    if horario:
       ...
       hora=st.slider('Selecione o horário',min_value=time.min,max_value=time.max,step=timedelta(minutes=1),format="hh:mm",disabled=True)
       #hora=st.slider('Selecione o horário', min_value=0, max_value=24, value= value=(datetime.now()-timedelta(hours=3)), step=None, format=None, key=None, help=None, on_change=None, args=None, kwargs=None, disabled=True)
    else:
